...
'''
# section s1
imgs1, noise, netG = roll_video(proj_path, desert_lbl, netG(z_dim+n_classes, img_length), n_classes, z_dim, lf, device, ratio, n_clips=int(225/(1/0.2)), step_size=0.2)
print(imgs1.shape)
imgs2, noise, netG = transit_video(desert_lbl, sea_lbl, n_classes, noise, netG, lf, ratio, device, step_size=0.2, z_step_size=0.25, l_step_size=0.25, transit_mode='scroll')
print(imgs2.shape)
imgs3, noise, netG = roll_video(proj_path, sea_lbl, netG, n_classes, z_dim, lf, device, ratio, n_clips=int(150/(1/0.2)), step_size=0.2, original_noise=noise)
print(imgs3.shape)
imgs4, noise, netG = transit_video(sea_lbl, forest_lbl, n_classes, noise, netG, lf, ratio, device, step_size=0.2, z_step_size=0.33, l_step_size=0.33, transit_mode='circular')
print(imgs4.shape)
imgs5, noise, netG = roll_video(proj_path, forest_lbl, netG, n_classes, z_dim, lf, device, ratio, n_clips=int(160/(1/0.25)), step_size=0.25, original_noise=noise)
print(imgs5.shape)
imgs6, noise, netG = transit_video(forest_lbl, desert_lbl, n_classes, noise, netG, lf, ratio, device, step_size=0.25, z_step_size=0.1, l_step_size=0.1, transit_mode='uniform')
print(imgs6.shape)
imgs7, noise, netG = roll_video(proj_path, desert_lbl, netG, n_classes, z_dim, lf, device, ratio, n_clips=int(168/(1/0.5)), step_size=0.5, original_noise=noise)
print(imgs7.shape)
'''
